...
function scaleToNose(multiPersonSegmentation, ctx) {
    ...
    function scaleAt(curx, cury, amount) { // at in screen coords amount is amount to scale
        scale *= amount;
        origin.x = curx - (curx - origin.x) * amount;
        origin.y = cury - (cury - origin.y) * amount;
    };
    ...
    scaleAt(cx, cy, amount); // scale 2 times at clickedPoint
...
function drawBokehEffect(canvas, image, multiPersonSegmentation, backgroundBlurAmount, edgeBlurAmount, flipHorizontal) {
    ...
    if (buttonOn) {
        scaleToNose(multiPersonSegmentation, ctx)
...
const main = async () => {
    const button = document.getElementById('bighead')
...
const animate = async () => {
    ...
    // console.log("PART:", partSegmentation)
    ...
    drawBokehEffect(
        canvas1, video, partSegmentation, backgroundBlurAmount,
        edgeBlurAmount, flipHorizontal);
    // bodyPix.drawPixelatedMask(
    //     canvas1, video, coloredPartImage, opacity, maskBlurAmount,
